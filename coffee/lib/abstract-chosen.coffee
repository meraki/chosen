class AbstractChosen
  ...
  handle_paste_multiple: (evt) ->
    ...
    if (content && content.match(/\n/))
      ...
      makeOrSelectOption(line) for line in lines

      @form_field_jq.trigger "chosen:updated"
      @form_field_jq.trigger "change", { add_options: lines }
  ...
  winnow_results: ->
    ...
    for option in @results_data
      ...
      if this.include_option_in_results(option)
        ...
        unless option.group and not @group_search
          ...
          exact_result = true if eregex.test option.search_text
    ...
    if results < 1 and searchText.length
      ...
      this.show_create_option( searchText ) if @options.create_option and not exact_result and searchText.length
